      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          <div class="relative">
            ...
            >
              <option
                v-for="opt in statusOptionsWithCounts"
                :key="opt.value"
                :value="opt.value"
              >
                {{ opt.display }}
          ...
          <div class="relative">
            ...
            >
              <option
                v-for="opt in priorityOptionsWithCounts"
                :key="opt.value"
                :value="opt.value"
              >
                {{ opt.display }}
    ...
    <div>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <!-- Lawyer Info -->
          <div
            v-if="caseItem.lawyerInfo"
            class="flex items-center gap-2 mb-3 p-2 bg-blue-50 rounded-md"
          >
            <div
              class="w-6 h-6 bg-blue-200 rounded-full flex items-center justify-center"
            >
              <User class="w-3 h-3 text-blue-600" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-xs font-medium text-blue-900 truncate">
                {{ caseItem.lawyerInfo.name }}
              </p>
          ...
          <div
            v-if="caseItem.case_points && caseItem.case_points.length > 0"
            class="mb-3 p-3 bg-blue-50 rounded-md border border-blue-100"
          >
            <div class="flex items-center gap-2 mb-2">
              <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
              </svg>
              <div class="text-xs font-medium text-blue-900">Case Progress Updates</div>
              <div class="text-xs text-blue-600">({{ caseItem.case_points.length }} point{{ caseItem.case_points.length !== 1 ? 's' : '' }})</div>
            </div>
            <div class="flex flex-wrap gap-1">
              <span
                v-for="point in caseItem.case_points.slice(-4)"
                :key="point.id"
                :class="[
              ...
              >
                +{{ caseItem.case_points.length - 4 }} more
              </span>
            </div>
            <div class="mt-2 text-xs text-blue-600">
              Last updated by your lawyer: {{ formatDate(caseItem.case_points[caseItem.case_points.length - 1]?.created_date) }}
            </div>
          </div>
          <div
            v-else-if="caseItem.status !== 'draft' && caseItem.status !== 'closed'"
            class="mb-3 p-2 bg-gray-50 rounded-md border border-gray-200 text-center"
          >
            <div class="text-xs text-gray-500">
              Case progress updates will appear here as your legal team works on your case
            </div>
          </div>
          ...
          <div class="space-y-1 mb-3">
            ...
            <div class="flex items-center gap-2 text-xs text-gray-500">
              ...
              <span
                >Updated:
                {{ formatDate(caseItem.updated_date || caseItem.created_date) }}</span
              >
            </div>
          </div>
...
const statusOptionsWithCounts = computed(() => {
  return statusOptions.map((o) => {
    const count =
      o.value === "all"
        ? cases.value.length
        : cases.value.filter((c) => c.status === o.value).length;
...
const priorityOptionsWithCounts = computed(() => {
  return priorityOptions.map((o) => {
    const count =
      o.value === "all"
        ? cases.value.length
        : cases.value.filter((c) => c.priority === o.value).length;
    return { ...o, count, display: `${o.label} (${count})` };
  });
});

const activeCasesCount = computed(() => {
  return cases.value.filter((c) => c.status === "open" || c.status === "in_progress")
    .length;
});
...
const getCaseOutcomeIndicator = (caseItem) => {
  ...
  if (score > 2) {
    status = 'winning';
    text = 'Case looking favorable';
  } else if (score < -2) {
    status = 'losing';
    text = 'Case facing challenges';
  }
...
const submitCase = async (caseItem) => {
  if (
    !confirm(
      "Are you sure you want to submit this case for legal review? Once submitted, it will be visible to administrators for lawyer assignment."
    )
  ) {
    return;
  }
  ...
  try {
    ...
    alert(
      "Case submitted successfully! An administrator will review it and assign a lawyer soon."
    );
  } catch (error) {
