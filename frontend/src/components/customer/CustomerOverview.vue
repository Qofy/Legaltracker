<template>
  <div class="space-y-6">
      ...
      <div>
        ...
        <p class="text-gray-500 mt-1">
          {{
            isLoading
              ? "Loading dashboard..."
              : "Welcome back! Here's an overview of your case"
          }}
        </p>
      </div>
      <button
        ...
        >
          <div class="flex items-start justify-between">
              ...
              >
                ...
                <div class="flex flex-wrap gap-1">
                  ...
                  >
                    ...
                    {{ point.description.slice(0, 15) }}{{ point.description.length > 15 ? '...' : '' }}
                  </span>
                  <span
            ...
            <button
              ...
              "
              class="ml-4 px-4 py-2 border border-blue-600 text-blue-600 rounded-md text-sm font-medium hover:bg-blue-50"
            >
              View Details
            </button>
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div class="space-y-3 max-h-64 overflow-y-auto">
          <div
            v-if="importantUpdates.length === 0"
          ...
          <div
            v-else
            v-for="update in importantUpdates"
            :key="update.id"
    ...
    >
      <div
        class="bg-white rounded-lg p-6 max-w-3xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col"
      >
...
const activeCases = computed(() => {
  return myCases.value.filter((c) => c.status === "open" || c.status === "in_progress")
    .length;
});
  ...
  try {
    ...
    myCases.value = allCases.filter((c) => {
      ...
      if (
        ...
        c.customer_ids.includes(userId)
      ) {
        return true;
      }
    ...
    if (myCases.value.length > 0) {
      ...
      try {
        ...
        unreadMessages.value = allMessages.filter(
          (m) => caseIds.includes(m.case_id) && !m.sent_by_customer && !m.is_read
        ).length;
      } catch (error) {
      ...
      try {
        ...
        recentDocuments.value = allDocuments
          ...
          .sort(
            (a, b) =>
              new Date(b.created_date || b.upload_date) -
              new Date(a.created_date || a.upload_date)
          )
          .slice(0, 5);
      } catch (error) {
      ...
      if (upcomingDeadlines.value > 0) {
        importantUpdates.value.push({
          id: "deadline-alert",
          message: `You have ${upcomingDeadlines.value} upcoming deadline${
            upcomingDeadlines.value > 1 ? "s" : ""
          } in the next 7 days`,
...
const handleCreateCase = async (caseData) => {
  try {
    ...
    alert(
      "Case request submitted successfully! A lawyer will review your case and contact you soon."
    );
