          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              ...
              <p class="text-sm text-gray-600 mb-3">
                From:
                <strong>{{ report.from_user?.full_name || "Unknown Lawyer" }}</strong>
              </p>
              ...
              <div v-if="report.report_data" class="bg-gray-50 rounded-md p-3 mb-3">
                ...
                <div class="text-sm text-gray-600 space-y-1">
                  ...
                  <div v-if="getReportData(report).format">
                    ...
                    {{ getReportData(report).format.toUpperCase() }}
                  </div>
                  <div v-if="getReportData(report).generated_at">
                    <strong>Generated:</strong>
                    {{ formatDate(getReportData(report).generated_at) }}
                  </div>
                </div>
              </div>
        ...
        <div class="p-6" v-if="selectedReport">
          <div class="space-y-4">
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                <strong>From:</strong>
                {{ selectedReport.from_user?.full_name || "Unknown" }}
              </div>
              <div>
                <strong>Date:</strong> {{ formatDate(selectedReport.created_date) }}
              </div>
              <div>
                <strong>Type:</strong>
                {{ getReportTypeLabel(selectedReport.message_type) }}
              </div>
            ...
            <div class="border-t pt-4">
              ...
              <div class="whitespace-pre-wrap text-gray-700 bg-gray-50 rounded-md p-4">
            ...
            <div v-if="selectedReport.report_data" class="border-t pt-4">
              ...
              <pre
                class="text-xs text-gray-600 bg-gray-50 rounded-md p-4 overflow-x-auto"
                >{{ formatReportData(selectedReport.report_data) }}</pre
              >
            </div>
...
const newReports = computed(
  () => reports.value.filter((r) => !r.status || r.status === "new").length
);
const caseReports = computed(
  () => reports.value.filter((r) => r.message_type === "case_report").length
);
const taskReports = computed(
  () => reports.value.filter((r) => r.message_type === "report").length
);
...
const loadReports = async () => {
  ...
  try {
    ...
    // try to attach lawyer objects if we already have them
    try {
      if (lawyers.value && lawyers.value.length) {
        const byId = new Map(lawyers.value.map((l) => [String(l.id), l]));
        reports.value = reports.value.map((r) => {
          if (!r.from_user && r.from_user_id) {
            const found = byId.get(String(r.from_user_id));
            if (found) return { ...r, from_user: found };
          }
          return r;
        });
      }
...
const loadLawyers = async () => {
  try {
    ...
    // enrich any loaded reports with lawyer objects
    try {
...
const downloadAttachment = (report) => {
  ...
  const safeBase64ToBlob = (b64Data, contentType = "") => {
    // Remove any surrounding whitespace/newlines
    const cleaned = String(b64Data).replace(/\s/g, "");
    // atob may throw if the string is not valid base64
  ...
  try {
    let { content, name, type } = report.file_attachment || {};

    if (!content) {
      throw new Error("No attachment content");
    }

    // If the attachment was stored as an object with a `data` field
    ...
    // Handle data URI (e.g., data:application/pdf;base64,....)
    if (typeof content === "string" && content.startsWith("data:")) {
      ...
      blob = safeBase64ToBlob(b64, type);
    } else {
      // Assume the content is raw base64 without the data: prefix
      blob = safeBase64ToBlob(content, type || "application/octet-stream");
    }
...
// Lifecycle
onMounted(async () => {
  // Load lawyers first so we can display reporter names immediately
  await loadLawyers();
  await loadReports();
});
</script>
